...
let FormSignUp =()=> {
  return generalDB.query("SELECT * FROM form_items INNER JOIN form_list ON form_items.form_id=form_list.id WHERE form_id=3");
}


module.exports.SignUpPageJson= async (req) => {
  return JSON.parse('{"Header":'+JSON.stringify(await header.Navbar(req))
  +',"Content":'+JSON.stringify(await FormSignUp())+
